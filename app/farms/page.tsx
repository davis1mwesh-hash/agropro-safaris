...
const slugs: Record<string, string> = {
  'Kirinyaga Coffee Estate': 'kirinyaga-coffee-estate',
  'Limuru Green Valley Farm': 'limuru-green-valley',
  'Thika Organic Gardens': 'thika-organic-gardens',
}
...
export default function FarmsPage() {
  ...
  const filtered = farms.filter(f => {
    const matchCat = active === 'All' || f.type === active
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
            ...
            <button key={c.name}
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap border transition ${
                active === c.name
                  ...
                  : 'bg-white text-gray-600 border-gray-200 hover:border-[#40916c]'
              }`}>
              {c.icon} {c.name}
            </button>
          ))}
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="text-sm text-gray-500">
            <strong className="text-gray-900">{filtered.length}</strong> farms available
          </p>
        ...
        {!loading && filtered.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-gray-500">No farms found. Try a different search or category.</p>
          </div>
        )}
        ...
        {!loading && filtered.length > 0 && (
          <div className="grid md:grid-cols-3 gap-6">
            {filtered.map(farm => {
              const slug = slugs[farm.name] || farm.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')
              ...
              return (
                <Link href={`/farms/${slug}`} key={farm.id}
                  ...
                  <div className="h-48 bg-gradient-to-br from-[#2d6a4f] to-[#52b788] flex items-center justify-center text-6xl relative">
                    {emoji}
                    <span className="absolute top-3 left-3 bg-white text-[#2d6a4f] text-xs font-bold px-3 py-1 rounded-full">
                      {farm.type} Farm
                    </span>
                  </div>
                  <div className="p-5">
                    ...
                    <div className="text-xs text-[#40916c] font-medium mb-3">
                      ✅ Open: {farm.open_days?.join(' · ')}
                    </div>
                    <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                      <div className="text-sm">
                        ⭐ <strong>{farm.avg_rating || '—'}</strong>
                        <span className="text-gray-400 ml-1">({farm.total_reviews || 0} reviews)</span>
                      </div>
        ...
        <div className="mt-16 bg-[#1a3d2b] rounded-2xl p-8 text-center">
          ...
          <Link href="/register-farm"
            className="bg-[#e9a825] text-white px-6 py-3 rounded-full font-semibold hover:bg-[#f4c84a] transition inline-block">
            🌾 Register My Farm
          </Link>
        </div>
